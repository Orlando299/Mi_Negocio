...
function renderVentas(filter='') {
  ...
  if(!data.length){ list.innerHTML = `<div class="empty"><div class="empty-icon">🔍</div><div class="empty-text">No se encontraron ventas</div></div>`; return; }
  list.innerHTML = data.map(v=>`
    <div class="sale-card" onclick="showToast('Venta ${v.id} — ${v.cliente}')">
...
function filterVentas(){ renderVentas(document.getElementById('venta-search').value); }
...
function renderInv(filter='') {
  ...
  if(!data.length){ list.innerHTML = `<div class="empty"><div class="empty-icon">📦</div><div class="empty-text">No se encontraron productos</div></div>`; return; }
  const stockLabel = {ok:'En stock', low:'Stock bajo', out:'Agotado'};
  list.innerHTML = data.map(p=>`
    <div class="inv-card" onclick="showToast('${p.nombre} · ${p.stock} unidades')">
...
function filterInv(){ renderInv(document.getElementById('inv-search').value); }

function renderClients(filter='') {
  const list = document.getElementById('client-list');
  const q = filter.toLowerCase();
  const data = clientes.filter(c => !q || c.nombre.toLowerCase().includes(q) || c.phone.includes(q));
  if(!data.length){ list.innerHTML = `<div class="empty"><div class="empty-icon">👥</div><div class="empty-text">No se encontraron clientes</div></div>`; return; }
  const tagLabel = {vip:'VIP', regular:'Regular', nuevo:'Nuevo'};
  list.innerHTML = data.map(c=>`
    <div class="client-card" onclick="showToast('${c.nombre} · ${c.pedidos} pedidos')">
...
function filterClients(){ renderClients(document.getElementById('client-search').value); }
